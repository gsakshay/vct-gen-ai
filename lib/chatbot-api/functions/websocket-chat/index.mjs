...
async function saveTeamComposition(session_id, user_id, input) {
  try {
    // Prepare the payload to get existing team composition
    ...
    const getSessionCommand = new InvokeCommand({
      FunctionName: sessionFunctionName,
      InvocationType: 'RequestResponse',
      Payload: Buffer.from(JSON.stringify(getSessionPayload)),
    });

    // Invoke Lambda to get existing team composition
    const existingTeamCompResponse = await lambdaClient.send(getSessionCommand);

    let existingTeamComp = null;
    if (existingTeamCompResponse.Payload) {
      const payloadString = Buffer.from(existingTeamCompResponse.Payload).toString();
    ...
    const saveTeamCommand = new InvokeCommand({
      FunctionName: sessionFunctionName,
      InvocationType: 'RequestResponse',
      Payload: Buffer.from(JSON.stringify(saveTeamPayload)),
    });
...
async function getTeamComposition(session_id, user_id) {
  try {
    ...
    const getTeamCommand = new InvokeCommand({
      FunctionName: sessionFunctionName,
      InvocationType: 'RequestResponse',
      Payload: Buffer.from(JSON.stringify(getTeamPayload)),
    });
