...
async function retrievePlayerList(region, tournament, agent, map) {
  ...
  table.find("tr").each((i, row) => {
    ...
    if (i == 0) {
      ...
      //   const cellContent = $(cell).text().trim(); // Default to text content

      //   // Check if there's an anchor tag in the cell
      //   const link = $(cell).find("a").attr("href");
      //   if (link) {
      //     rowData.push(link); // If <a> tag exists, push the href (URL)
      //   } else {
      //     rowData.push(cellContent); // Otherwise, push the text content
      //   }
      // });
      // data.push(rowData);
      ...
      const player = {
        ...
        agents: [],
        rnd: $(cells[2]).text().trim(),
        r2: $(cells[3]).text().trim(),
        acs: $(cells[4]).text().trim(),
        kd: $(cells[5]).text().trim(),
        kast: $(cells[6]).text().trim(),
        adr: $(cells[7]).text().trim(),
        kpr: $(cells[8]).text().trim(),
        apr: $(cells[9]).text().trim(),
        fkpr: $(cells[10]).text().trim(),
        fdpr: $(cells[11]).text().trim(),
        hs_pct: $(cells[12]).text().trim(),
        cl_pct: $(cells[13]).text().trim(),
        cl: $(cells[14]).text().trim(),
        kmax: $(cells[15]).text().trim(),
        k: $(cells[16]).text().trim(),
        d: $(cells[17]).text().trim(),
        a: $(cells[18]).text().trim(),
        fk: $(cells[19]).text().trim(),
        fd: $(cells[20]).text().trim(),
      }
          ...
          if (parsedChunk) {
            ...
            if (usingTool) {
              ...
              if (toolUse.name == "query_db") {
                ...
                const players = await retrievePlayerList(toolUse.region,toolUse.tournament,toolUse.agent, toolUse.map);
