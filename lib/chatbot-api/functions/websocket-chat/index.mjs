...
const SYS_PROMPT = process.env.PROMPT + process.env.PROMPT2;
...
/**
 * Saves the map for a given session and user.
 * @param {string} session_id - The session identifier.
 * @param {string} user_id - The user identifier.
 * @param {object} mapData - The map data to save.
 * @returns {object} - The response payload from the Lambda function.
 */
async function saveMap(session_id, user_id, mapData) {
  try {
    const saveMapPayload = {
      body: JSON.stringify({
        operation: 'save_map',
        maps: mapData,
        session_id: session_id,
    ...
    const saveMapCommand = new InvokeCommand({
      FunctionName: sessionFunctionName,
      Payload: JSON.stringify(saveMapPayload),
    });

    const saveResponse = await lambdaClient.send(saveMapCommand);
    ...
    console.error('Error invoking save_map:', error);
    throw error;
  }
}
...
/**
 * Retrieves the map for a given session and user.
 * @param {string} session_id - The session identifier.
 * @param {string} user_id - The user identifier.
 * @returns {object} - An object containing the map data or an error payload.
 */
async function getMap(session_id, user_id) {
  try {
    const getMapPayload = {
      body: JSON.stringify({
        operation: 'get_map',
    ...
    const getMapCommand = new InvokeCommand({
      FunctionName: sessionFunctionName,
      Payload: JSON.stringify(getMapPayload),
    });

    const response = await lambdaClient.send(getMapCommand);
    const payloadString = Buffer.from(response.Payload).toString();
    const payload = JSON.parse(payloadString);

    if (payload.statusCode === 200) {
      const maps = JSON.parse(payload.body);
      return {
        statusCode: 200,
        maps: maps,
      };
    ...
    console.error('Error invoking get_map:', error);
    throw error;
  }
}
        ...
        for await (const event of stream) {
          ...
          if (parsedChunk) {
            ...
            if (usingTool) {
              ...
              } else if (toolUse.name == "save_team_composition") {
              ...
              } else if (toolUse.name == "get_team_composition") {
              ...
              } else if (toolUse.name == "save_map") {
                console.log("saving maps!")
                const saveMapResult = await saveMap(data.session_id, data.user_id, toolUse.input.maps);
                toolResult = saveMapResult.message || "Maps saved successfully.";
              } else if (toolUse.name == "get_map") {
                console.log("retrieving maps!")
                const getMapResult = await getMap(data.session_id, data.user_id);
                toolResult = JSON.stringify(getMapResult.maps) || "Maps not found.";
              }
