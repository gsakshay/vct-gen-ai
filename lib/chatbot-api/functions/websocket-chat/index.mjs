...
const getUserResponse = async (id, requestJSON) => {
  try {
    ...
    let history = claude.assembleHistory(lastFiveMessages, "Please answer this new question. Enclose thought processes in <thinking> tags. For queries related to forming a new team, when naming agents, enclose them in <agent> tags with relevant properties and players should get a <player> tag.".concat(userMessage))
