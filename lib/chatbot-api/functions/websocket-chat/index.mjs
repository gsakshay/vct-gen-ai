...
async function retrievePlayers(tournament, sortBy = "games_played") {  
  const QUERY = `
  select player_name, sum(total_kills) as total_kills, sum(total_deaths) as total_deaths, sum(games_played) as games_played, avg(average_kills) as average_kills, avg(average_deaths) as average_deaths
  from vct_stats_db
