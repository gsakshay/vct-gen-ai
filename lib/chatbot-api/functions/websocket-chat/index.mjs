...
async function retrievePlayers(sortBy = "games_played") {  
  const QUERY = `
  select player,avg(avg_kills) as avg_kills, avg(avg_assists) as avg_assists, avg(avg_deaths) as avg_deaths, avg(avg_score) as avg_score, avg(kdr) as avg_kdr, sum(games_played) as games_played
  from international_final
  group by 
  player
  order by
  ${sortBy} desc;
  ...
  return result.map(player => {
      return `Player: ${player.player},  Avg Kills: ${player.avg_kills}, Avg Assists: ${player.avg_assists}, Avg Deaths: ${player.avg_deaths}, Avg Score: ${player.avg_score}, KDR: ${player.kdr}, Games Played: ${player.games_played}`;
  }).join('\n');
}
...
async function retrievePlayerInfo(playerID) {
  ...
  return result.map(player => {
      return `Player: ${player.player}, Agent Name: ${player.agent_name}, Avg Kills: ${player.avg_kills}, Avg Assists: ${player.avg_assists}, Avg Deaths: ${player.avg_deaths}, Avg Score: ${player.avg_score}, KDR: ${player.kdr}, Games Played: ${player.games_played}`;
  }).join('\n');
          ...
          if (parsedChunk) {
            ...
            if (usingTool) {
              ...
              if (toolUse.name == "query_db") {
                ...
                const players = await retrievePlayers(toolUse.input.sort_by)
                toolResult = players;
