...
async function returnTeamJSON(modelOutput) {
  ...
  ```
  const jsonResponse = jsonModel.getResponse(jsonModelSystemPrompt,[],"Based on this response, return a JSON object representing a team.".concat(modelOutput))
  let responseParams = {
    ConnectionId: id,
    Data: "<TEAM_INFO>"
  }  
...
const getUserResponse = async (id, requestJSON) => {
  try {
    ...
    let history = claude.assembleHistory(lastFiveMessages, "Please answer this new question. Enclose thought processes in <thinking> tags. For queries related to forming a new team, players should get a <player> tag with a player_index, a name, and agent property.".concat(userMessage))
